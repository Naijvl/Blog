...
function getfootlocation($slug, $type="page") { //slug
...
function recent_comments_mobile($limit=10, $count=28, $avatarsize=26) {
	global $wpdb;
	$sql = "SELECT ID, comment_ID, comment_content, post_title, comment_date, comment_author_email, comment_author FROM $wpdb->posts, $wpdb->comments WHERE $wpdb->posts.ID=$wpdb->comments.comment_post_ID AND post_status = 'publish'  AND comment_type = ''";
	$sql .= "AND comment_approved = '1' ORDER BY $wpdb->comments.comment_date DESC LIMIT $limit";
	$comments = $wpdb->get_results($sql);
	...
	foreach ($comments as $comment) {
		...
		$comment_date_str1 = $comment_date->format("Y-m-d");
		$comment_date_str2 = $comment_date->format("h:i:s");
		// print_r( Date("Y-m-d"));
		// exit();
		if($comment_date_str1 == Date("Y-m-d")){
			$comment_date_str1 = $comment_date_str2;
		}
		$output .= '<li><a href=' . $permalink . '><div class="author">' .get_avatar($comment, $avatarsize)  . '</div><div class="comment"><p>'.$comment_author .'</p>' . $comment_excerpt . '<div class="comment_date">'.$comment_date_str1.'</div></div></a></li>';
	}
	echo $output;
}


function catch_all_lazy_image_mobile($num) {
	global $post, $posts;
	...
	$img_num = count($matches[0]);
	if(empty($img_num)){
		$default_img = "<img data-original ='/images/default.jpg' class='lazy' width='90' height='90'>";
		echo $default_img;
...
function catch_all_bg() {
	global $post, $posts;

	ob_start();
	ob_end_clean();
	$output = preg_match_all('~src="(.*?(jpg|jpeg|gif|png))~', $post->post_content, $matches);
	$img_num = count($matches[0]);

	for($i = 0;$i < $img_num;$i++){
		$bg = $matches[0][$i];
		$bg = str_replace(array("src","\"","="),"",$bg);
		echo "<li style='background-image:url(".$bg.")'>"."</li>";
	}	
