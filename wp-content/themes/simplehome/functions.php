...
function exclude_category( $query ) {
    ...
    if ( $query->is_home() && $query->is_main_query()&& !$query->get( 'cat' ) ){
       $query->set( 'category__not_in',$display_categories);
    }
}

//菜单回调函数
function Bing_nav_fallback(){
	echo '<div class="menu-header"><ul><li>'.__( '<a href="wp-admin/admin.php?page=thset">请在 "后台 - 外观 -菜单" 设置导航菜单</a>','Bing' ).'</ul></li></div>';
}
...
include(TEMPLATEPATH.'/includes/admin/miued.php'); 
include(TEMPLATEPATH.'/includes/admin/types_music.php');
include(TEMPLATEPATH.'/includes/admin/music_meta_box.php');
include(TEMPLATEPATH.'/includes/admin/focus_admin.php');
include(TEMPLATEPATH.'/includes/admin/thumbnails-display_meta_box.php');
