...
function check(){
jQuery.get("<?php bloginfo('template_url');?>/includes/admin/getAjax.php", function(result){
	getArr = result.split('|');
...
$Archives = $_SERVER['DOCUMENT_ROOT'].$twodir.'/wp-content/uploads/simplehome/log.txt';	     //Log文件
$Folder   = $_SERVER['DOCUMENT_ROOT'].$twodir.'/wp-content/uploads/simplehome/';     //下载目录
$password = @file_get_contents("http://1.miued.sinaapp.com/sm_updata.php?key=getsm");//管理密码
//var_dump($password);
?>
<!--简单控制地址长度-->
...
<p><span>提示：如果您无法通过后台自动更新进行升级，请手动<a href="http://www.miued.com/2510/" target="_blank">下载最新版本</a>主题通过FTP进行覆盖升级。</span><br /></p>
<div id="result"></div>
<hr />
<br />
...
if (is_writable($filename)) //判断是否可写
{
	...
	else
    {
        ...
        {
			$archive = new PclZip($_SERVER['DOCUMENT_ROOT'].$twodir.'/wp-content/uploads/simplehome/'.basename($url));
...
while($cnt < 3 && ($contents=@file_get_contents('http://1.miued.sinaapp.com/themes_news_simplehome.php', false, $context))===FALSE){
	$cnt++;
	}
 echo $contents;
 ?>
