	jQuery(function($){	
		...
		var toWider = function(){
			...
			$(".container-mu").css('margin-left', '60px');
		...
		var toNarrow = function(){
			...
			$(".container-mu").css('margin-left', '320px');
			$('.list').css('left', listHideLeft);
			$('.weibo-show').css('left', weiboHideLeft);
		...
		//公告
		if(jQuery('#g_box li')[0] && $('#g_box li').length > 1){
		jQuery('.gdb').fadeIn();
		var tId = setInterval(gg_roll,5000)
			}else if(jQuery('#g_box li')[0]){
				jQuery('.gdb').fadeIn();
		}
...
function gg_roll(){
	//alert(1);
		jQuery('#g_box').animate({'top':'-30px'},600,function(){jQuery('#g_box').css('top','0');move_GG_li()});
}
function move_GG_li(){
